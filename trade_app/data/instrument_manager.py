...
from config.symbols import SPOT_INDICES, OPTION_INDICES
...
class InstrumentManager:
    ...
    def get_expiries(self, index_name):
        ...
        config = OPTION_INDICES[index_name]
        
    
        print(
            df[
                (df["name"] == config["name"]) &
                (df["segment"] == config["segment"])
            ][["name", "tradingsymbol", "segment"]].head(20)
        )
    ...
    def get_atm_option(
        ...
        if row.empty:
            print(
                df[(df["instrument_type"] == option_type)
                ]["strike"].sort_values().unique()
            )
